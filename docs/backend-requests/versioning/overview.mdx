...
3. **Dashboard setting**: Set a default version in the [Clerk Dashboard](https://dashboard.clerk.com/last-active?path=api-keys). This version will be used if one is not explicitly specified.
...
Webhook events will always use the API version set in the Dashboard. You can reply any events that fail due to the version switch.
